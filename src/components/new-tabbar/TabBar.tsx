...
import type { Event, Layout, NavigationState, Route, Scene, SceneRendererProps } from './types';
...
const getTranslateX = (scrollAmount: Animated.Value, maxScrollDistance: number) =>
    Animated.multiply(
...
const getTabBarWidth = <T extends Route>({
    ...
    return routes.reduce<number>(
        (acc, _, i) =>
            acc +
            (i > 0 ? gap ?? 0 : 0) +
            getComputedTabWidth(i, layout, routes, scrollEnabled, tabWidths, flattenedTabWidth),
        0,
    );
};
...
const getScrollAmount = <T extends Route>({
    ...
    const centerDistance = Array.from({
        ...
        return (
            total +
            (navigationState.index === i ? (tabWidth + (gap ?? 0)) / 2 : tabWidth + (gap ?? 0))
        );
    }, 0);
...
export function TabBar<T extends Route>({
    ...
    const hasMeasuredTabWidths =
        ...
        routes.slice(0, navigationState.index).every(r => typeof tabWidths[r.key] === 'number');
    ...
    const translateX = React.useMemo(
        () => getTranslateX(scrollAmount, getMaxScrollDistance(tabBarWidth, layout.width)),
                    ...
                    ? (e: LayoutChangeEvent) => {
                          ...
                          if (
                              ...
                              routes
                                  ...
                                  .every(r => typeof measuredTabWidths.current[r.key] === 'number')
                          ) {
    ...
    const handleViewableItemsChanged = useLatestCallback(
        ({ changed }: { changed: ViewToken[] }) => {
            ...
            if (
                item.isViewable &&
                (index % 10 === 0 || index === navigationState.index || index === routes.length - 1)
            ) {
                setTabWidths({ ...measuredTabWidths.current });
            }
        },
